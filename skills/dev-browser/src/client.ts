...
function inferModeFromServerUrl(
  serverUrl: string
): "standalone" | "extension" | "electron" | undefined {
  try {
    const { port } = new URL(serverUrl);
    if (port === String(DEFAULT_PORTS.extension)) return "extension";
    if (port === String(DEFAULT_PORTS.standalone)) return "standalone";
    if (port === String(DEFAULT_PORTS.electron)) return "electron";
  } catch {
    // Ignore parse errors; fall back to default mode.
  }
  return undefined;
}
...
export async function connect(
  ...
  let modeFromUrl: "standalone" | "extension" | "electron" | undefined;

  if (typeof serverUrlOrOptions === "string") {
    serverUrl = serverUrlOrOptions;
    opts = options;
    modeFromUrl = inferModeFromServerUrl(serverUrl);
  } else if (serverUrlOrOptions) {
    opts = serverUrlOrOptions;
    const port = DEFAULT_PORTS[opts.mode ?? "extension"];
  ...
  // Default to extension mode unless explicitly overridden.
  const mode = opts.mode ?? modeFromUrl ?? "extension";
